...
const TakePictureComp = ({navigation})=>{
...
//        Alert.alert("Taking picture")
        if(camera){
            const options = {quality:0.1, exif:true }
    ...
    const renderItem = ({item, index})=>{
            return(
                <View style={{marginVertical:8, marginHorizontal:2}}>
                    <TouchableOpacity onPress={() => imgPress(item, index)}>
                        {
                            (curImageIndx == index)?(
                            <Image source={{uri:item}} style={{width:70, 
                                height:70, borderRadius:3, borderWidth:4,
                                borderColor:'orange'
    ...
    const savePictures = async ()=>{
       // Alert.alert('Saving the picture')
        try{
            const jsonVal = JSON.stringify(image)
            await AsyncStorage.setItem('@storage_img', jsonVal)
        ...
        <View style={{flex:1}}>
            ...
            <View style={{flex:0.3}}>
                <View style={{flexDirection:"row"}}>
                    <TouchableOpacity style={[styles.vw_btn,{flex:0.5,alignItems:'center', flexDirection:'row'}]} onPress={()=> navigation.goBack()}>
                    ...
                    <View style={[styles.vw_btn,{flexDirection:'row', flex:0.2, alignItems:'center'}]}>
                        <TouchableOpacity onPress={()=> type == CameraType.back? setType(CameraType.front):setType(CameraType.back)}>
                            <MaterialIcons name="flip-camera-android" size={24}
                                            style={{alignSelf:"center"}}
                                        color="black" />
                        </TouchableOpacity>
                    </View>
                    <TouchableOpacity onPress={() => savePictures()}
                        style={[styles.vw_btn, {flexDirection:'row', flex:0.5, alignItems:'center', backgroundColor:'#facf66'}]}
                    >
                        <MaterialIcons name="save-alt" size={24} color="black" style={{alignContent:'center'}} />
...
export default TakePictureComp;
